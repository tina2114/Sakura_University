...
void CgenClassTable::code_global_data()
{
  ...
  str << STRINGTAG << LABEL 
      << WORD <<  *class_to_tag_table.lookup(string) 
      << endl;    
}
...
//
// The CgenClassTable constructor actually does all of the work of 
// code generation: builds the inheritance graph, computes layout information,
// and codes each class.
//
...
{ }
//
// Initialize a CgenNode X using information from another CgenNode
// (the parent of X).
//
		    ...
		    SymbolTable<int,Entry> apt)
{
  ...
  for(; child; child = child->tl()) {
     child->hd()->init(num_methods,
                          ...
                          var_binding_table,
                          attribute_proto_table);
     children = new List<CgenNode>(child->hd(),children);    // Reverse tree so traversed in the same order as old code
  }
  //
...
//
//  Code the dispatch table of a class, followed by the 
// dispatch tables of the class' children.
//
...
//
//  This code is called by 'new' after the prototype has been copied.
...
//  dipatch table pointer with the real dispatch table and execute the
//  slot initializations.
//
//  The pointer to the object is in ACC. 
//
