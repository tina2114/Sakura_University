...
class SymtabEntry {
private:
  SYM id;        // the key field  关键领域
  DAT *info;     // associated information for the symbol  符号的相关信息
public:
...
public:
   ...
   // Create pointer to current symbol table.  创建指向当前符号表的指针
   ...
   //输入一个新的范围。 符号表被组织为列表列表。 列表的头是最内部的作用域，
   // 尾部是外部的作用域。 必须先输入范围，然后才能将任何内容添加到表中。

   void enterscope()
   {
       // The cast of NULL is required for template instantiation to work
       // correctly.
       //必须使用NULL强制转换才能使模板实例化正常工作。
   ...
   ScopeEntry *addid(SYM s, DAT *i)
   {
       ...
       // 搞了一个信息域的id，该id是另一个存有se的id和头部指针的信息域，然后是一个尾部指针
