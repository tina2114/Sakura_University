...
/* This must be invoked with the same arguments as all the other phases, in 
 * particular a .cl and (optionally) a -o flag. It will append the 
...
int main(int argc, char *argv[]) {
  ...
  if (!out_filename && optind < argc) {   // no -o option
      ...
      //cerr << "Will emit annotations to " << out_filename << "\n";
  }

  ast_yyparse();

  // Now open the output file for append
  ofstream out(out_filename, ofstream::out | ofstream::app);
  emitAnnotations(out);
  out.close();
}
