...
{ 
    if(state_sema[i]==HUNGRY&&state_sema[LEFT]!=EATING
            &&state_sema[RIGHT]!=EATING) // 当前哲学家处于饥饿状态，左右两位哲学家不处于进食状态
    {
        state_sema[i]=EATING; // 当前哲学家进入进食状态
        up(&s[i]); // 离开临界区，将当前进程唤醒
    }
}
...
void phi_test_condvar (i) { 
    if(state_condvar[i]==HUNGRY&&state_condvar[LEFT]!=EATING
        ...
        state_condvar[i] = EATING ; // 状态变为eating
...
void phi_put_forks_condvar(int i) {
     down(&(mtp->mutex)); // 进入临界区
        state_condvar[i] = THINKING; // 状态变为思考
        phi_test_condvar(LEFT); // 检测左边哲学家能否进餐
        phi_test_condvar(RIGHT); // 检测右边哲学家能否进餐
//--------into routine in monitor--------------
     // LAB7 EXERCISE1: YOUR CODE
     // I ate over
     // test left and right neighbors
//--------leave routine in monitor--------------
     if(mtp->next_count>0) // 如果有哲学家沉睡就唤醒
        up(&(mtp->next));
     else
        up(&(mtp->mutex)); // 离开临界区
}
